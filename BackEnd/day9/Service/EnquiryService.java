...
public class EnquiryService {
    ...
    public EnquiryModel saveEnquiryDetails(EnquiryModel enquiry) {
        List<Object[]> courses = enquiryRepository.getByCourseId((int) enquiry.getCourses().getCourse_id()); // Assuming
                                                                                                             // 1 is the
                                                                                                             // course
                                                                                                             // ID you
                                                                                                             // want to
        ...
        if (courses.isEmpty()) {
            throw new RuntimeException(
                    "No course found for the given course ID : " + enquiry.getCourses().getCourse_id());
        }
    ...
    public EnquiryModel updateEnquiryDetails(int id, EnquiryModel updatedEnquiry) {
        Optional<EnquiryModel> existingEnquiryOptional = enquiryRepository.findById(id);
